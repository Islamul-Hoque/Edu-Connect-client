    ...
    <div className=" px-6 md:px-10 py-6 md:py-10 ">
      <div className="max-w-7xl mx-auto ">
        <motion.header className="text-center mb-14" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={fadeInUp}>
          ...
          <p className="text-xl text-gray-600 dark:text-gray-50 w-[90%] md:w-[85%] mx-auto">We are more than just a platform—we are the trusted connector for education in Bangladesh.</p>
        </motion.header>

        <div className="grid md:grid-cols-2 gap-8 pb-14">
          <motion.div className="p-6 bg-white SectionCard rounded-xl shadow-lg" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <h3 className="text-2xl font-semibold mb-3 text-center">Our Mission</h3>
            <p className="text-gray-700 dark:text-gray-50 text-center">We aim to simplify tuition finding by providing a trusted platform where students can connect with verified tutors quickly and securely.</p>
          </motion.div>
          <motion.div className="p-6 bg-white SectionCard rounded-xl shadow-lg" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <h3 className="text-2xl text-center font-semibold mb-3">Our Vision</h3>
            <p className="text-gray-700 dark:text-gray-50 text-center">To make quality education accessible for everyone by bridging the gap between learners and educators through technology.</p>
          </motion.div>
        </div>
        ...
        <div className="mb-20">
          ...
          <motion.div className="grid md:grid-cols-3 gap-8" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={staggerContainer}>
            {coreValues.map((value, idx) => (
              <motion.div key={idx} className="p-6 text-center bg-white SectionCard rounded-xl shadow-lg transition duration-300 hover:shadow-xl hover:scale-[1.05]" variants={fadeInUp}>
                <div className="flex justify-center mb-3 text-indigo-500">{value.icon}</div>
                <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-50 mb-2">{value.title}</h3>
                <p className="text-gray-600 dark:text-gray-50 text-sm">{value.desc}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
